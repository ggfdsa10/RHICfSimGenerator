c 27.10.2024 Link routines between QGSJet-III and EPOS.
c author T. Pierog


c-----------------------------------------------------------------------
      subroutine IniQGSJETIII
c-----------------------------------------------------------------------
c Primary initialization for QGSJET-III
c-----------------------------------------------------------------------
     ...
     *,bqgs2,bmaxqgs,bmaxnex,bminnex
      ...
      call utpri('iniqgsIII',ish,ishini,6)
      write(ifmt,'(a,i6)')'initialize QGSJET-III ...'
...
c common initialization procedure
      DATDIR='qgsjetIII/'      !default if nothing is defined
      call qgaini(DATDIR)
      BMAXNEX=dble(bmaxim)
      BMINNEX=dble(bminim)
      egymin=0.1
      ...
      call utprix('iniqgsIII',ish,ishini,6)
      end

c-----------------------------------------------------------------------
      subroutine IniEvtQGSIII
c-----------------------------------------------------------------------
...
c QGSJET-III Common
      ...
      common /qgsIIInex1/xa(iapmax,3),xb(iapmax,3)
     *,bqgs,bmaxqgs,bmaxnex,bminnex


      if(matarg.gt.iapmax.or.maproj.gt.iapmax)
     &  call utstop('Nucleus too big for QGSJET-III (Mmax=208) !&')
      call iclass(idproj,iclpro)
      call iclass(idtarg,icltar)
      icp=idtrafo('nxs','III',idproj)
...
c Important to calculate e0 (kinetic energy) in double precision
      ...
      bmax=BMAXQGS
      qgsIIIincs=qgsIIIcrse(ekin,maproj,matarg,idtargin)
      if(engy.lt.egymin)qgsIIIincs=0.          !below egymin, no interaction
      call xsigma                             !change bm in qgfz
      bkmx=sqrt(sigine/10./pi)        !10= fm^2 -> mb
      if(ish.ge.2)write(ifch,*)
     &  'QGSJET-III used with (E,proj,maproj,matarg,bmax)',e0,icp
     &  ,maproj,matarg,bmax
...
c-----------------------------------------------------------------------
      subroutine emsqgsIII(iret)
c-----------------------------------------------------------------------
c  call qgsjet-III to simulate interaction
c-----------------------------------------------------------------------
...
c particle masses for proper boost
      double precision am0,amn,amk,amsig,amlam,ameta
     *,dmmin,dmres,wdres
      ...
      if(a.gt.0..and.rangen().gt.qgsIIIincs/10./a)goto 1001   !no interaction
      if(ish.ge.3)call alist('Determine QGSJET-III Production&',0,0)
...
c keep the projectile spectators as fragments
     ...
     $       ' Fragment from qgsjetIII ',nptl,' id :',idptl(nptl)
...
c 0 - pi0, 1 - pi+, -1 - pi-, 2 - p, -2 - pbar, 3 - n,
...
c       20 - sigma+, -20 - sigma+bar, 21 - sigma-, -21 - sigma-bar
     ...
     $     , ' momentum : ',(esp(k,is),k=1,4)
            ...
            id=idtrafo('III','nxs',ic)
            if(ish.ge.7)write(ifch,'(a,i5,a,i5,a)')
     $       ' epos particle ',nptl,' id :',id,' after conversion'
            call idmass(id,amepo)
            am=amepo         !just in case particle is not in the following list
            ica=abs(ic)
            if(ica.le.1)then
              am=am0
            elseif(ica.eq.2.or.ica.eq.3)then
              am=amn
            elseif(ica.eq.4.or.ica.eq.5)then
              am=amk
            elseif(ica.eq.6)then  !lambda
              am=amlam
            elseif(ica.eq.20.or.ica.eq.21)then
              am=amsig
            elseif(ica.eq.10)then   !eta
              am=ameta
            elseif(ica.eq.16.or.ica.eq.17)then  !rho
              am=dmmin(1)
            elseif(ica.eq.7.or.ica.eq.8)then  !delta
              am=dmmin(2)
            elseif(ica.eq.18.or.ica.eq.19)then  !K*
              am=dmmin(3)
            endif
            esum=esum+esp(1,is)
...
c-----------------------------------------------------------------------
     ...
     *            ,xtarg(mamx),ytarg(mamx),ztarg(mamx)
      ...
      call utpri('cqgsIII ',ish,ishini,4)  
...
1000  continue
      ...
      call utprix('cqgsIII ',ish,ishini,4)
      return
...
c------------------------------------------------------------------------------
      function qgsIIIcrse(egy,mapro,matar,id)
c------------------------------------------------------------------------------
c inelastic cross section of qgsjet-III 
...
c------------------------------------------------------------------------------
      ...
      if(id.eq.0)then
        do k=1,3
          mt=int(airanxs(k))
          qgsIIIcrse=qgsIIIcrse+airwnxs(k)*qgsect(egyl,icpro,mapro,mt)
        enddo
      else
        qgsIIIcrse=qgsect(egyl,icpro,mapro,matar)
      endif
