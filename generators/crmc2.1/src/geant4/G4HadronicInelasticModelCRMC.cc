...
G4HadronicInelasticModelCRMC::G4HadronicInelasticModelCRMC(int model, const G4String& modelName):
  G4HadronicInteraction(modelName), fPrintDebug(false)
{
	...
	std::string crmc_param = GetCrmcParamPath();        //"crmc.param"; // CRMC default, see CRMCoptions.cc in the CRMC package
	
	fInterface = new CRMCinterface();
	fInterface->init(model);

	// Patch from Nicola Mori
	// Convert C strings to Fortran strings
 	// FIXME: this buffer is not deleted and thus generates a memory leak; however it is not clear how long the Fortran code
	//        will make use of the string, so for now let's live with this small leak.
	char *crmc_param_fstr = new char[1000]; // The buffer size is matched with the string size in Fortran code (see crmc_init_f subroutine)
	char *output_fstr = new char[1000]; // Empty string
	try {
		ConvertToFortran(crmc_param.c_str(), 1000, crmc_param_fstr);
		ConvertToFortran("", 1000, output_fstr);
	} catch(std::runtime_error &err) {
		delete[] crmc_param_fstr;
		throw &err;
	}

	// open FORTRAN IO at first call
	fInterface->crmc_init(MAX_ENERGY_CMS_GEV,seed,model,produce_tables,fTypeOutput,crmc_param_fstr,output_fstr,0);
...
std::string G4HadronicInelasticModelCRMC::GetCrmcParamPath(){
	std::string crmcParamPath = "";
	crmcParamPath = getenv (CRMC_CONFIG_FILE_ENV_VARIABLE);
	if (crmcParamPath==""){
...
// This function is added as a part of a Patch from Nicola Mori
void G4HadronicInelasticModelCRMC::ConvertToFortran(const char* cstring, std::size_t fstring_len, char* fstring){
	std::size_t inlen = std::strlen(cstring);
	std::size_t cpylen = std::min(inlen, fstring_len);
	if (inlen > fstring_len){
		throw std::runtime_error("[G4HadronicInelasticModelCRMC::ConvertToFortran]: Fortran string buffer size (" + 
		std::to_string(fstring_len) + " is smaller that C string (size: " + std::to_string(inlen));
	}
	std::copy(cstring, cstring + cpylen, fstring);
	std::fill(fstring + cpylen, fstring + fstring_len, ' ');
}

G4HadronicInelasticModelCRMC::~G4HadronicInelasticModelCRMC()
{}

G4HadFinalState * G4HadronicInelasticModelCRMC::ApplyYourself(const G4HadProjectile &aTrack, G4Nucleus &targetNucleus){
	...
	while(true){
		...
		for(int i=0; i<gCRMC_data.fNParticles;i++){
			...
			double m  = pdef->GetPDGMass()/GeV;
			e_final  += std::sqrt(m*m + p2);
		}
		...
		if(!ENERGY_NON_CONSERVATION_RESAMPLE){
			...
			std::cout<< "#                                                                                  #" << std::endl;
			std::cout<< "# [G4HadronicInelasticModelCRMC::ApplyYourself]: Energy non conservation detected: #" << std::endl;
			std::cout<< "# e_initial = " << e_initial << " GeV"                                                << std::endl;
			std::cout<< "# e_final   = " << e_final   << " GeV"                                                << std::endl;
	...
	for(int i=0; i<gCRMC_data.fNParticles;i++){ 
		...
		if(!pdef) if(IGNORE_PARTICLE_UNKNOWN_PDGID){
			std::cout<<std::endl;
			std::cout<<"********************************************************************************************************"<<std::endl;
			std::cout<<" -- WARNING ----------------------------------------------------------------------------------- WARNING --  "<<std::endl;
			std::cout<<" [G4HadronicInelasticModelCRMC] Geant4 could not find particle definition for PDG ID = " << gCRMC_data.fPartId[i] << std::endl; 
			std::cout<<" [G4HadronicInelasticModelCRMC] Ignoring this particle. This might cause energy non-conservation!"      << std::endl; 
			std::cout<<" -- WARNING ----------------------------------------------------------------------------------- WARNING --  "<<std::endl;
			std::cout<<"********************************************************************************************************"<<std::endl;
			continue;
		}
			...
			std::cout<<" [G4HadronicInelasticModelCRMC] Geant4 could not find particle definition for PDG ID = " << gCRMC_data.fPartId[i] << std::endl; 
			std::cout<<" [G4HadronicInelasticModelCRMC] Throwing exception! Please report to: " << ERROR_REPORT_EMAIL   << std::endl; 
			std::cout<<" -- ERROR ----------------------------------------------------------------------------------- ERROR --  "<<std::endl;
			std::cout<<"********************************************************************************************************"<<std::endl;
			throw;
		}
...
G4bool G4HadronicInelasticModelCRMC::IsApplicable (const G4HadProjectile &a, G4Nucleus &b){
	return true;
}

G4ParticleDefinition* G4HadronicInelasticModelCRMC::GetParticleDefinition(long particle_id,int& error_code){ 
...
bool G4HadronicInelasticModelCRMC::IsMultiNeutron(int Z, int A){
	bool result = false;
	if (!Z && A>1){ 
		if(A<= SPLIT_MULTI_NEUTRONS_MAXN){
			...
			std::cout<<" [G4HadronicInelasticModelCRMC::IsMultiNeutron] ERROR A="<<
...
void G4HadronicInelasticModelCRMC::SplitMultiNeutrons(CRMCdata& CRMC_data){
	for(int i=0; i<CRMC_data.fNParticles;i++){
		...
		int pdef_errorcode;
		G4ParticleDefinition* pdef = GetParticleDefinition(CRMC_data.fPartId[i],pdef_errorcode);
		...
		int A = (particle_id - CRMC_ION_COEF_0 - CRMC_ION_COEF_Z*Z)/CRMC_ION_COEF_A;
		if(Z!=0 || A<2){
			std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] ERROR consistency check failed! Throwing exception! " << std::endl;
			throw;
		}

		//
		std::cout<<std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO splitting the floowing particle into neutrons: " << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    Z = " << Z << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    A = " << A << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartId = " << CRMC_data.fPartId[i] << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartPx = " << CRMC_data.fPartPx[i] << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartPy = " << CRMC_data.fPartPy[i] << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartPz = " << CRMC_data.fPartPz[i]  << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartEnergy = " << CRMC_data.fPartEnergy[i]  << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartMass   = " << CRMC_data.fPartMass[i]  << std::endl;
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] INFO    fPartStatus = " << CRMC_data.fPartStatus[i]  << std::endl;
		...
		if(e_n<m_n){
			std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] WARNING neutron energy " << 
		...
		//
		std::cout<<" [G4HadronicInelasticModelCRMC::SplitMultiNeutrons] done for a particle. " << std::endl;
		std::cout<<std::endl;
	}
}
