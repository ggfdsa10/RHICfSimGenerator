...
c check if energy is conserved before boost
      if(iappl.eq.1)then
        diff=abs(p2(3)-p1(3))
        scal=p2(4)/p1(4)
c        if(abs(scal-1.).le.errlim.and.abs(diff/psoll).lt.errlim
c     &     .and.(iLHC.eq.0.or.
c     &    (abs(p2(1)).lt.errlim.and.abs(p2(2)).lt.errlim)))then
c        if(ish.ge.4) 
c     & write (ifch,'(2x,a,2g14.6)') 'Energy OK: ',scal,abs(diff/psoll)
c          goto 9999
c        else
c         diff=0.
c         scal=1.
c        endif
        if(abs(scal-1.).le.errlim.and.abs(diff/psoll).lt.errlim ) then
           if (iLHC .eq.0. ) then
              if(ish.ge.4) 
     &   write (ifch,'(2x,a,2g14.6)') 'Energy OK: ',scal,abs(diff/psoll)
              goto 9999
           else if (abs(p2(1)).lt.errlim.and.abs(p2(2)).lt.errlim) then
              if(ish.ge.4) 
     &   write (ifch,'(2x,a,2g14.6)') 'Energy OK: ',scal,abs(diff/psoll)
              goto 9999
            else
